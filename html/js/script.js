$(document).on('click', '.accept-button', function(e) {
    ...
    $.post('https://prime-pizza/Click', JSON.stringify({}))
    if (PriceAmount != '' && PriceAmount != undefined && NoteAmount != '' && NoteAmount != undefined) {
        $.post('https://prime-pizza/AddPrice', JSON.stringify({Price: PriceAmount, Note: NoteAmount}))
        $.post('https://prime-pizza/CloseNui', JSON.stringify({}))
        ...
        $.post('https://prime-pizza/ErrorClick', JSON.stringify({}))
    }
});
...
$(document).on('click', '.payment', function(e) {
    ...
    if (NumberId != null && Note!= null && Price != null) {
        $.post('https://prime-pizza/PayReceipt', JSON.stringify({Price: Price, Note: Note, Id: NumberId}))
        $.post('https://prime-pizza/CloseNui', JSON.stringify({}))
        $(".main-container").animate({"top": "-30vh"}, 250, function() {
            $('.items').html('');
            $('.main-container').css("display", "none");
